 ...
 *      enum membership, and the remove-from-actor refine() are all enforced
 *      (bad input throws and never reaches the bridge).
 *   2. forwarding + response shaping — the bridge method name + payload the
 *      handler sends, and the object it builds from the bridge result.
 *
