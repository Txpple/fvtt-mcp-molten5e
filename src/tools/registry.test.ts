...
describe('tool registry', () => {
  it('advertises 70 uniquely-named tools (matches the documented surface)', () => {
    const { tools } = build();
  ...
  it('advertises the update-actor tool by name', () => {
    const { tools } = build();
    expect(new Set(tools.map(t => t.name)).has('update-actor')).toBe(true);
  });
