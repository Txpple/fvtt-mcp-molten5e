...
export class SceneTools {
  ...
  async handleCreateTeleporter(args: any): Promise<string> {
    const parsed = CreateTeleporterSchema.parse(args ?? {});
    const result = await this.foundry.call('createSceneTeleporter', parsed);
    if (result?.notFound) {
  ...
  async handleCreateRegion(args: any): Promise<string> {
    const parsed = CreateRegionSchema.parse(args ?? {});
    const result = await this.foundry.call('createSceneRegions', parsed);
    if (result?.notFound) {
      return `Scene not found: "${result.notFound}". No regions created.`;
    }
    const lines = Array.isArray(result?.regions)
      ? result.regions.map((r: any) => `\n  • ${r.id} — ${r.name}`).join('')
      : '';
    return `Created ${result?.created ?? 0} region(s) on "${result?.sceneName}" (${result?.sceneId})${lines}`;
  }

  async handleListRegions(args: any): Promise<any> {
    const parsed = ListRegionsSchema.parse(args ?? {});
    const result = await this.foundry.call('listSceneRegions', parsed);
    if (result?.found === false) {
      return `Scene not found: "${result?.notFound ?? parsed.sceneIdentifier}".`;
    }
    return result;
  ...
  async handleUpdateRegion(args: any): Promise<string> {
    const parsed = UpdateRegionSchema.parse(args ?? {});
    const result = await this.foundry.call('updateSceneRegion', parsed);
    if (result?.updated === false) {
      return `Region not found: "${result?.notFound ?? parsed.regionId}". Nothing changed.`;
    }
    const shape = result?.region?.shapes?.[0];
    const shapeStr = shape
      ? ` — ${shape.type}${shape.width !== undefined ? ` ${shape.width}×${shape.height}px @ (${shape.x},${shape.y})` : ''}`
      : '';
    return `Updated region ${result?.region?.id} on "${result?.sceneName}" (${result?.sceneId})${shapeStr}.`;
  }

  async handleDeleteRegion(args: any): Promise<string> {
    const parsed = DeleteRegionSchema.parse(args ?? {});
    const result = await this.foundry.call('deleteSceneRegions', parsed);
    if (result?.notFound) {
      return `Scene not found: "${result.notFound}". Nothing deleted.`;
    }
    const missing =
