...
describe('tileDescriptor.toCreateDoc', () => {
  ...
  it('errors (isolated, not thrown) on a missing required field', async () => {
    expect((await tileDescriptor.toCreateDoc!({ x: 0, y: 0, width: 1, height: 1 })).error).toMatch(
      /src/
    );
    expect(
      (await tileDescriptor.toCreateDoc!({ src: 'a.png', x: 0, y: 0, width: 1 })).error
    ).toMatch(/height/);
  });
});
...
describe('tileDescriptor.buildPatch', () => {
  ...
  it('reports changed:false when only the id is supplied', async () => {
    const r = await tileDescriptor.buildPatch!({}, { id: 't1' });
